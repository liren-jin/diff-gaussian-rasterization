	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		variances.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_opacity, out_uncertainty, importance_score, radii, n_touched, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dvariances = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  W, H, 
	  means3D.contiguous().data<float>(),
	  variances.contiguous().data<float>(),
	  ...
	  dL_dvariances.contiguous().data<float>(),
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacities, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dvariances);
}
