...
renderCUDA(
	...
	const float* __restrict__ dL_dpix_depths,
	const float* __restrict__ dL_dpix_uncertainties,
	...
	float* __restrict__ dL_dvariance)
{
	...
	__shared__ float collected_variances[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_variances[block.thread_rank()] = variances[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			atomicAdd(&dL_dvariance[global_id], dL_dpixel_uncertainty * (alpha * T) * (alpha * T));
